        ...
        <div class="fondoDerechoCrud">
            <div class="crudDerecho">
                <div class='TituloCrud'>
                    <h1>Datos de Docentes</h1>
                </div>
                <?php

                $Bd = conexionBaseDatos();

                $query = "SELECT d.id, p.nombrePers, p.apellidoPers, p.fechanacimientoPers, p.cuilPers, p.sexoPers, p.nacionalidadPers, p.provinciaPers, p.dniPers, u.userUsu, u.estadorolUsu, preRol.nombreRol,
                dom.BarrioDom, dom.CalleDom, dom.AlturaDom, c.correoCont, c.telefonoCont
                FROM docentes d
                JOIN personas p ON d.personas_Pers_id = p.idPers
                JOIN usuario u ON d.usuario_Usu_Id = u.IdUsu
                JOIN preestablecerrol preRol ON u.PreestablecerRol_PreestablecerRol_id = preRol.idPreRol
                JOIN domicilio dom ON dom.personas_Pers_id = p.idPers
                JOIN contacto c ON c.personas_Pers_id = p.idPers
                WHERE d.EliminacionLogDoc = 'A'";
